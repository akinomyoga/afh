/*
	このソースコードは [afh.Parse.Design.dll] afh.Design.TemplateProcessor によって自動的に生成された物です。
	このソースコードを変更しても、このソースコードの元になったファイルを変更しないと変更は適用されません。

	This source code was generated automatically by a file-generator, '[afh.Parse.Design.dll] afh.Design.TemplateProcessor'.
...
namespace afh.Rendering{
	public partial struct ColorName{
		...
		private static double[] read_func_args(string text,ref int i){
			...
			{args[iArgs++]=double.Parse("0");if(iArgs==NumberOfArgs)mode=SKIP_MODE;}
						break;
			...
			{args[iArgs++]=double.Parse(word);if(iArgs==NumberOfArgs)mode=SKIP_MODE;}
						mode=AFT_MODE;
						break;
